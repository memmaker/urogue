...
static int obj_tile(struct object *o)
{
    int w = o->o_which;
    switch (o->o_type) {
    case WEAPON: return w >= 0 && w < (int)(sizeof weap_tile / sizeof *weap_tile) ? weap_tile[w] : -1;
    case ARMOR:  return w >= 0 && w < (int)(sizeof armor_tile / sizeof *armor_tile) ? armor_tile[w] : -1;
#ifdef MM
    case MM:     return w >= 0 && w < (int)(sizeof mm_tile / sizeof *mm_tile) ? mm_tile[w] : -1;
#endif
#if defined(RELIC)
    case RELIC:  return w >= 0 && w < (int)(sizeof relic_tile / sizeof *relic_tile) ? relic_tile[w] : -1;
#elif defined(ARTIFACT)
    case ARTIFACT: return w >= 0 && w < (int)(sizeof relic_tile / sizeof *relic_tile) ? relic_tile[w] : -1;
#endif
    case FOOD:   return w >= 0 && w < (int)(sizeof food_tile / sizeof *food_tile) ? food_tile[w] : -1;
...
{
    int i;
    for (i = 1; i < (int)(sizeof mon_tile / sizeof *mon_tile); i++)
        ...
        if (1)
            for (l = mlist; l; l = next(l)) {
                ...
                if (tp->t_pos.y == y && tp->t_pos.x == x && tp->t_type == ch
                    && tp->t_index >= 0 && tp->t_index < (int)(sizeof mon_tile / sizeof *mon_tile))
...
#ifdef MAXPACK
    mvwprintw(p, y, 0, "%d/%d items, %ld gold", inpack, MAXPACK, (long) purse);
#else
    mvwprintw(p, y, 0, "%ld gold", (long) purse);
#endif
    wclrtoeol(p);
