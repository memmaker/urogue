/*
 * UltraRogue in the browser: draws the panes of the curses shim (port/be_web.c
 * calls Module.xr), keyboard input, tiling windows, sound, saves in
 * IndexedDB. Loaded before urogue-core.js.
 */
(function () {
	'use strict';
	...
	var DIR = '/urogue/save';                         /* IDBFS mount: saves, scores, layout */
	var SAVE = DIR + '/urogue.sav', LAYOUT_FILE = DIR + '/web-layout.json';
	...
	function crashed(err) {
		...
		console.error('[urogue] crash:', err);
	...
	window.addEventListener('error', function (e) {
		...
		if (e.error instanceof WebAssembly.RuntimeError || /urogue-core/.test(e.filename || '')) crashed(e.error || e.message);
	});
